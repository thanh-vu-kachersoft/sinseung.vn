    ...
    async function fetchSearchResults() {
      ...
      try {
        ...
        const res = await fetch(
          `/api/products?page=${page}&per_page=12&search=${encodeURIComponent(query)}${langParam}`,
        );
    ...
    <div className="bg-background min-h-screen">
      ...
      <section className="relative w-full">
        <div className="relative w-full h-62.5 md:h-auto md:aspect-1920/450 overflow-hidden flex items-center justify-center">
          ...
          />
          <h1
            className="relative z-10 text-[#C8102E] text-5xl md:text-6xl font-bold uppercase tracking-wide text-center px-4"
          ...
          <div className="max-w-[1400px] mx-auto w-full">
            <ul className="flex flex-wrap md:flex-nowrap">
              <li className="flex-1 min-w-[200px]">
                <div className="block text-center text-[#F5F5F0] py-[23px] text-[20px] font-medium transition-all duration-[600ms] outline-none bg-[#C8102E]">
                  {language === "vi"
                    ? "KẾT QUẢ TÌM KIẾM"
                    : language === "ko"
                      ? "검색 결과"
          ...
          <>
            <ul className="grid grid-cols-1 md:grid-cols-3 gap-[2%] gap-y-12.5">
              {products.map((product) => (
                <li
                  key={product.id}
                  className="group bg-[var(--background)] shadow-[5px_5px_10px_0_#eaeaea] overflow-hidden"
                >
                  <NextLink
                    href={product.link}
                  ...
                  <div className="p-[20px_25px_25px]">
                    <NextLink
                      href={product.link}
                      className="block text-[18px] font-medium text-[#333] mb-[50px] overflow-hidden text-ellipsis whitespace-nowrap transition-all duration-[600ms] hover:text-[#C8102E]"
                    >
